...
function learndash_notifications_meta_box_output( $key, $args ) {
	...
	if ( 'delay_unit' === $key && empty( $key_value ) ) {
		...
		if ( strlen( $key_value ) < 1 || $key_value == 0 ) {
			$key_value = 'all';
		} elseif ( is_numeric( $key_value ) && $key_value > 0 ) {
			// Set pre-selected value if it's available in database.
			$pre_selected_object = get_post( $key_value );
	
			$pre_selected_option = '<option value="' . $key_value . '" selected="selected">' . $pre_selected_object->post_title . '</option>';
		}
			...
			<div class="sfwd_option_div">
				...
				<?php if ( $args['type'] == 'dropdown' ) : ?>
					...
					<?php foreach ( $args['value'] as $value => $title ) : ?>
                        <option value="<?php echo $value; ?>" <?php selected( $value, $key_value ); ?>><?php echo $title; ?></option>
					<?php endforeach; ?>

					<?php 
						if ( isset( $pre_selected_option ) && ! empty( $pre_selected_option ) ) {
							echo $pre_selected_option;
						}
					?>
